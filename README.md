...
- Описание сервиса агентного уровня с диаграммой последовательности
...
### Вопросы для защиты
1. Что такое очередь? Что такое прикладной уровень? Почему они так называются. 
...
Необходимо разработать систему обмена текстовыми сообщениями/файлами/телеметрией в реальном времени, состояющую из трех уровней: прикладной, транспортный и агентный. Каждый из уровней реализуется отдельным веб-сервисом. Для каждого варианта нужно передавать в JSON через все три уровня данные по вашей теме (начальные координаты, временной отрезок, название и тд) - эти поля будут только у вашего варианта и называются по вашей теме в Swagger.
...
При получении сегментов от агентного уровня они помещаются в очередь, раз в N секунд собираются в единое сообщение и передаются на прикладной уровень. Если часть из сегментов не была принята (1-3 цикла не было новых сегментов по этому сообщению), то на прикладной уровень итоговое сообщение передается с признаком ошибки.
...
Сервис должен терять сегменты от агентного уровня (перед внесением в очередь) с вероятностью R %, где R номер варианта (от 1 до 10).
...
Текстовые сообщения, телеметрия или файлы разбиваются на сегменты по X байт и отправляются отдельными сегментами от агентного на транспортный уровень. 
